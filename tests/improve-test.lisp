...
(deftest restricted-catalogue-signals-on-removed-op
  (let ((cat (cap:make-catalogue :world)))
    ...
    (let* ((restricted (make-restricted-catalogue cat))
      ...
      (ok (find 'send-message (cap:capability-operations orig)
                :key #'cap:capability-operation-name))
      (ok (null (find 'send-message (cap:capability-operations stub)
                      :key #'cap:capability-operation-name)))
      (ok (signals (cap:invoke-operation stub 'send-message "a" "b")
